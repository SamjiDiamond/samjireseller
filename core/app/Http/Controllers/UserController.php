            ...
            if($user->pin==$request->pin){
                if($user->acct_no!=$request->acct_no){
                    if($user->balance>$amountx || $user->balance==$amountx){
                        ...
                        if(!$rep['status']){
                            return back()->with('alert', 'Error, kindly confirm your account details');
                        }
                        ...
                        $fields = [
                            ...
                            "recipient" => $rep['data']['recipient_code']
                        ];
                        $fields_string = http_build_query($fields);
                        //open connection
                        $ch = curl_init();
                        ...
                        if($resp['status']){
                            ...
                            return back()->with('alert', 'Server error, kindly contact support');
                        }
